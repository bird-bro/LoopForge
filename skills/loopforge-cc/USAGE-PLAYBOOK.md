...
**AI**:
1. 跑 `./scaffold.sh check .`（快速审计，25 项 = 3 环境自检 + 22 结构合规）。
...
3. 输出审计结果 + 已知项说明：
   ...
   - hooks 为 echo 桩（设计如此）-> 随时替换为真实 lint/format 闸门（PASS stub）
...
> `check` 是 CLI 快照(25 项可自动判定:3 环境自检 + 22 结构合规,含 O7 CJK 扫描、S5 Superpowers 检测(含用户级插件缓存)、O4 归档错位检测);完整 33 项审计(含 S3 工作流、H4 工作树等需语义判断的项)请让 AI 进 skill audit 模式。
>
> **审计判断指引**（AI 33 项审计中的语义判断项）：
> - **H4 git worktree**：多仓库 workspace（根目录非 git，各子目录独立 git）是合法模式 -> WARN，不是 FAIL
> - **S7/H10 hooks**：scaffold 生成的 echo 桩是设计占位 -> PASS (stub)，替换为真实闸门是后续优化
